...
class BonLivraisonController extends Controller
{
    ...
    public function getAllBonLivraisons(){
        $AllBonLivraisons = Bonlivraison::leftJoin('status', 'status.id_status', '=', 'bon_livraisons.fk_status_bl')
    ...
    public function getPrixArticle_bl($fk_article){
     ...
     public function getBonLivraison($id_bl){
        ...
        $bonlivraisons = Bonlivraison::leftJoin('comptes', 'bon_livraisons.fk_compte_bl', '=', 'comptes.id_compte')
            ->leftJoin('status', 'bon_livraisons.fk_status_bl', '=', 'status.id_status')
            ->select('bon_livraisons.*', 'comptes.*','status.*')
            ->where('type_operation_bl','=',$request->type_operation_bl)
...
$height = 300-$y*1.5;*/
 ...
 PDF::writeHTMLCell(0, 0, '',$y,$calculeHtml , 0, 1, 0, true, '', true);
     ...
     //  PDF::writeHTMLCell(0,0,'',0,$test1, 1, 0, 0, false, '', true);
      ...
      public function pdf_a($reference_bl){
        $bonLivraison= Bonlivraison::leftJoin('comptes', 'bonLivraisons.fk_compte_bl', '=', 'comptes.id_compte')
        ->leftJoin('contacts', 'contacts.fk_compte_comp', '=', 'comptes.id_compte')
        ->leftJoin('macompagnies', 'comptes.fk_compagnie', '=', 'macompagnies.id_compagnie')
        ->leftJoin('mode_paiements', 'bonLivraisons.reference_bl', '=', 'mode_paiements.fk_document')
      ...
      if (isset($bonLivraison[0]->fk_bonCommande)) {
       ...
       $objethtml = '<p style="margin-top: 50px;">Ref Bon Cammande  :'. $bonLivraison[0]->fk_bonCommande.'</p>';
        ...
        foreach ( $commandes as $commande ){
           ...
           $infoComp = ''.$bonLivraison[0]->nom_compte.' <span>
           </span>'.$bonLivraison[0]->raison_social.'<span> 
           RC N°: </span>'.$bonLivraison[0]->RC.'<span>
           Secteur Activité: </span>'.$bonLivraison[0]->secteur_activite.'<span>
           E-mail: </span>'.$bonLivraison[0]->email.'<span>
           Site: </span>'.$bonLivraison[0]->site_web.'<span>
           fax: </span>'.$bonLivraison[0]->fax.'<span>
           fix: </span>'.$bonLivraison[0]->fixe.'<span>
           GSM: </span>'.$bonLivraison[0]->portable.'<span>';
...
<tr>
    ...
    Tél: '.$bonLivraison[0]->portable.' /Fax: '.$bonLivraison[0]->faxe.'<br>
    '.$bonLivraison[0]->raison_sociale.'<br>
